from flask import Flask, redirect, send_file, request, render_template, jsonify, url_for, session
...
DATABASE_URL = "http://dbserver:5000"
...
def search_boat():
    ...
    boats = requests.post(DATABASE_URL+"/searchBoats", json={"email": email}).json()["boats"]
...
def search_stay():
    ...
    stays = requests.post(DATABASE_URL+"/searchStay", json={"stay_data": stay_date}).json()["stays"]
